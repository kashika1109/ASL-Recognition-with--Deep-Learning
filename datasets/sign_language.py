...
def load_data(container_path='datasets', folders=['A', 'B', 'C'],
    ...
    x_train = np.array(x[:int(len(x) * (1 - test_split))]) #training group of images
    y_train = np.array(y[:int(len(x) * (1 - test_split))]) #training labels of images
    x_test = np.array(x[int(len(x) * (1 - test_split)):])  #testing group of images
    y_test = np.array(y[int(len(x) * (1 - test_split)):]) #training labels of images
